...
$air=new klas_air;
$koneksi = $air->koneksi();
// $key = mysqli_query($koneksi, "SELECT nik, username, password, tipe_user FROM user ORDER BY nama ASC");
// while($data = mysqli_fetch_array($key)){
//     $nik  = $data[0];
//     $user = $data[1];
//     $pass = $data[2];
//     $tipe = $data[3];
//     echo "nik: $nik | user: $user | pass: $pass | tipe: $tipe <BR>";
        ...
        <div class="forms-wrap">
          <form autocomplete="off" class="sign-in-form needs-validation" method="post">
            ...
            if (isset($_POST['tombol'])) {
              ...
              $qc = mysqli_query($koneksi, "SELECT username, password, tipe_user FROM user WHERE username = '$username'");
